...
The dashboard shows live windows, the currently selected media session, audio sinks, and audio sources. Inactive windows in the taskbar strip can be clicked to request focus. The media card appears between windows and audio, shows artwork or app icon when available, uses centered icon-only transport buttons, and exposes a draggable progress bar with elapsed and total track time. Sink volume plus sink/source mute controls use HTTP control endpoints; source volume is read-only until the backend supports writes for sources.
