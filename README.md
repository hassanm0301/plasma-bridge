...
- [clients](clients/): React web dashboard and Flutter mobile app
...
The backend currently serves audio snapshots and controls, current media-session snapshots and transport controls, and Plasma window snapshots plus activation. The React dashboard renders those as a taskbar-style windows strip, a current-media card with artwork, large transport icons, draggable progress, and elapsed/total time, plus sink/source controls. The Flutter app renders the same runtime state in a denser Breeze-like layout optimized for landscape tablet use, with full-width windows and current-media sections.
